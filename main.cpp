...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	//振り子の先端の位置
	Vec3 point{ 0.0f,pendulum.anchor.y - pendulum.length,0.0f };
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (startFlag == true) {
			pendulum.anglarAcceleration = -(gravityAcceleration / pendulum.length) * std::sinf(pendulum.angle);
		...
		Matrix44 sphereWorldMatrix = MakeAffineMatrix({ 1.0f,1.0f,1.0f }, {0.0f,0.0f,pendulum.angle }, { 0.0f,-0.05f,0.0f }) * pointWorldMatrix;
		...
		Vec3 start = Transform(pendulum.anchor, viewProjectionMatrix * viewportMatrix);
